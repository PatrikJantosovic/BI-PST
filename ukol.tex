...
(1b) Pro každou skupinu zvlášť otestujte na hladině významnosti 5\% hypotézu, zda je střední hodnota rovná hodnotě K (parametr úlohy - 16), proti oboustranné alternativě. Můžete použít buď výsledek z předešlého bodu, nebo výstup z příslušné vestavěné funkce vašeho softwaru.
\\\\
\(H_0\) : střední hodnota je rovná 16\\
\(H_A\) : střední hodnota není je rovná 16\\
...
\end{itemize}
\par
Je zrejmé, že testovaná hodnota K = 16 v konfidenčním intervalu neleží. Na hladině spolehlivosti 5\% zamítneme \(H_0\) v prospěch  \(H_A\).  (platí pro oba sledované skupiny)
